      ...
      var render = function render(res) {
        ...
        for (var j = 0, len2 = res.list.length; j < len2; j++) {
          ...
          for (var i = 0, len = data.link.length; i < len; i++) {
            ...
            var minSrc = '/ImagesFolder/' + data.link[i];
            var src = '/ImagesFolder/' + data.link[i];
            ...
            liTmpl += '<figure class="thumb" itemprop="associatedMedia" itemscope="" itemtype="http://schema.org/ImageObject">\
                  <a href="' + src + '" itemprop="contentUrl" data-type="' + type + '" data-target="' + src + '">\
                    <img class="reward-img" data-type="' + type + '" data-src="' + minSrc + '" src="' + minSrc + '" itemprop="thumbnail" onload="if(this.naturalWidth&&this.naturalHeight){this.parentNode.setAttribute(\'data-size\', this.naturalWidth+\'x\'+this.naturalHeight);}">\
                  </a>\
      ...
      var initPhotoSwipeFromDOM = function initPhotoSwipeFromDOM(gallerySelector) {
        ...
        var parseThumbnailElements = function parseThumbnailElements(el) {
          ...
          for (var i = 0; i < numNodes; i++) {
            ...
            var imgChild = linkEl.querySelector ? linkEl.querySelector('img') : linkEl.getElementsByTagName('img')[0];
            var realW = (imgChild && imgChild.naturalWidth > 0) ? imgChild.naturalWidth : 0;
            var realH = (imgChild && imgChild.naturalHeight > 0) ? imgChild.naturalHeight : 0;

            size = linkEl.getAttribute('data-size') ? linkEl.getAttribute('data-size').split('x') : null;
            if ((!realW || !realH) && size && size.length >= 2) {
              realW = parseInt(size[0], 10);
              realH = parseInt(size[1], 10);
            }
            if (!realW || isNaN(realW)) realW = 1440;
            if (!realH || isNaN(realH)) realH = 1440;
        ...
        var openPhotoSwipe = function openPhotoSwipe(index, galleryElement, disableAnimation, fromURL) {
          ...
          // 动态高保真监听：当大图加载完成后，若实际宽高与预设不符，自动无感校正视口比例与原图清晰度
          gallery.listen('gettingData', function(index, item) {
            if (!item.hasRealSize && item.src && item.type !== 'video') {
              var img = new Image();
              img.src = item.src;
              img.onload = function() {
                if (this.naturalWidth && this.naturalHeight && (item.w !== this.naturalWidth || item.h !== this.naturalHeight)) {
                  item.w = this.naturalWidth;
                  item.h = this.naturalHeight;
                  item.hasRealSize = true;
                  gallery.invalidateCurrItems();
                  gallery.updateItemHTML(item);
                }
              };
            }
          });
